...
用户问"在 X 方面排第几"时，**把 X 原样传给 `--subset`，不要自己翻译**。脚本内置三层模糊匹配：

1. **精确别名命中**（90+ 个 alias）：`代码` `数学` `中文` `推理` `创作` `逻辑` `全球排名` `知识储备` `便宜` 等都会展开成对应 board 列表
2. **剥噪音再匹配**：用户说"编程**能力**"/"代码**方面**"/"数学**方面表现**"/"长文本**理解**"/"做**数学**题"，脚本会剥掉 `能力/方面/表现/理解/做/代/全球/整体` 等无意义修饰后再查
3. **双向子串扫描**：用户输入比 alias 长（如"前端开发" ⊃ "前端"）或更短（"code" ⊂ "coding"）都能命中

如果三层都没命中，脚本会自动推荐 Top-5 最接近的关键词。**支持的别名**（节选）：
...
**完整别名表**：见 `query/_common.py` 的 `BOARD_KEYWORD_ALIASES`（90+ 个映射，含 8 种语言、3 类行业、所有 AA 14 个 metric、以及"创作/逻辑/知识储备/便宜/全球排名"等口语化说法）。

**实测覆盖的用户说法**（全部一次命中，无需重问）：
- 总榜类：`总榜` `综合` `总分` `全球排名` `整体排名` `综合排名` `总排名` `intelligence`
- 代码类：`代码` `编程` `编程能力` `代码方面` `代码这块` `coding` `code` `前端` `前端开发` `react`
- 数学类：`数学` `代数学` `做数学题` `数学方面表现` `math` `aime`
- 中文类：`中文` `chinese` `中文水平` `中文表现`
- 推理类：`推理` `逻辑` `逻辑推理` `推理能力` `难题` `专家` `知识储备`
- 速度/价格：`速度` `速度怎么样` `延迟` `价格` `便宜` `便宜程度`
- 创作类：`创作` `创意` `写作` `创意写作`
